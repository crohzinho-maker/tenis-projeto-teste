...
botoesMenu.forEach(function (botao){
  botao.addEventListener("click",function (){ 
    botoesMenu.forEach(function (outroBotao){
      botao.classList.add("botao-cabecalho")
      outroBotao.classList.remove("botao-cabecalho")
    })
    ;
    })
})
...
botoesMenu.forEach(function (botao){
  botao.addEventListener("click", function (){
    const botaoClicadocima = botao.dataset.categoria;

    produtos.forEach(function (produto){
      const categoriaProduto = produto.dataset.categoria;

      const mesmaCategoria = categoriaProduto === botaoClicadocima;

      produto.style.display = mesmaCategoria ? "" : "none";
    })
  })
})
